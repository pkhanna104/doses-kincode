...
accuracy = load('data/rom_error_preeya.mat'); 
% Dont need this here; 
precision = load('data/precision_error_preeya.mat'); 

for s = 1:length(subjects)
    input = subjects{s};

    % Seems to be calc in msq_error_all_subjects_normals, saved in a file called 'input'_res.mat
    filename = strcat(path_to_data, 'data/', input, '_res.mat');
    subj_data = load(filename);
        
    % Function is below 
    [vartest_u,vartest_a,error_u,error_a] = acc_prec_test...
        (input, ...
        subj_data.u_rom_2_pinch,... % UNAFF 10 trials x 12 jt angles -- unaffected std of time series joint angle data of trial 
        subj_data.a_rom_2_pinch,... % AFF -- affected std of time series joint angle data of trial 
        accuracy.accuracy_data,...
        subj_data.un_raw,... %No idea whta this is 10 x 12
        subj_data.aff_raw,...%No idea whta this is 10 x 12
        precision.jt_error_all,...
        subj_data.median_trial_data_u,...
        subj_data.median_trial_data_a);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for  m = [1:11]  %skipping analysis of shoulder roll
        ...
        % Comparison of std to accuracy dat a
