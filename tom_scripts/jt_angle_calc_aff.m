function [zscore_mean, zscore_std,u_height,unaffect_all,data_palm,force,unaffect,u_time,sam_p_sec] = jt_angle_calc_aff(unaffected,angle_struct,input);
...
% determined by trial and error :(
    ...
    for n = 2:s(2)
        ...
        index3(n) = index2(1);
        index4 = find(round(unaffected.T_mat,1)  == round(unaffected.T_state(n),1))+1000; %80 (C9K,B8M,B12J), 250, 800 (all normals except sb), or 1200 (sb) 
    ...
    for n = 2:2:20;  %bc of the way task_state is organized!!
        trial{m} = index3(n)-10:index3(n+1)+70;  %-10 to 70 except W16H %start of button press to start of next button press
        m = m + 1;
    end
